...
def start():
	...
	host = raw_input(C+'[*]'+W+'asm>[dns_propagation] Host: ') # Target
	dom = flib(host) # Bad protocol [http:// https://]
	if chost(dom) == True: # Check For Host
		def psend(host):
			send = urlopen('http://www.viewdns.info/propagation/?domain='+host).read() # Send Request
		...
		try:
			...
			fname = rand+'dns_propagation_'+dom.replace('.','_')+'.html' # File Name
			recv = psend(dom)
