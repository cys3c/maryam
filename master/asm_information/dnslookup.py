...
def start():
	...
	host = raw_input(C+'[*]'+W+'asm>[dnslookup] Host/ip: ') # Target
	file = raw_input(C+'[*]'+W+'asm>[dnslookup] show File output?y/n ') # y = Save To File n = Show Recv
	dom = flib(host)
	xdom = 'https://api.hackertarget.com/dnslookup/?q='+dom
	if chost(dom) == True: # Check For Host
		...
		try:
			...
			if (file == 'y' or file == 'Y'):
				...
				fname = rand+'dnslookup_'+dom.replace('.','_')+'.html' # File Name
