...
def start():
	...
	host = raw_input(C+'[*]'+W+'asm>[ping] Host/ip: ') # Target
	file = raw_input(C+'[*]'+W+'asm>[ping] show File output?y/n ') # y = Save To File n = Show Recv
	dom = flib(host)
	xdom = 'https://api.hackertarget.com/nping/?q='+dom
	if chost(dom) == True: # Check For Host
		...
		try:
			...
			if (file == 'y' or file == 'Y'):
				...
				fname = rand+'ping_'+dom.replace('.','_')+'.html' # File Name
