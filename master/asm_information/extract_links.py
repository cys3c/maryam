...
def start():
	...
	host = raw_input(C+'[*]'+W+'asm>[extract_links] Host: ') # Target
	file = raw_input(C+'[*]'+W+'asm>[extract_links] show File output?y/n ') # y = Save To File n = Show Recv
	dom = flib(host)
	xdom = 'https://api.hackertarget.com/pagelinks/?q='+dom
	if chost(dom) == True: # Check For Host
		...
		try:
			...
			if (file == 'y' or file == 'Y'):
				...
				fname = rand+'extract_links_'+dom.replace('.','_')+'.html' # File Name
				fsave = saved(str(rQx),fname) # Save To File
				if(fsave==True):
