...
def start():
	...
	host = raw_input(C+'[*]'+W+'asm>[trace_ip] Host/ip: ') # Target
	file = raw_input(C+'[*]'+W+'asm>[trace_ip] show File output?y/n ') # y = Save To File n = Show Recv
	dom = flib(host)
	xdom = 'https://api.hackertarget.com/mtr/?q='+dom
	if chost(dom) == True: # Check For Host
		...
		try:
			...
			if (file == 'y' or file == 'Y'):
				...
				fname = rand+'trace_ip_'+dom.replace('.','_')+'.html' # File Name
