...
def start():
	...
	host = raw_input(C+'[*]'+W+'asm>[reverse_whois] Host/ip: ') # Target
	dom = flib(host) # Bad protocol [http:// https://]
	if chost(dom) == True: # Check For Host
		def psend(host):
			send = urlopen('http://www.viewdns.info/reversewhois/?q='+host).read() # Send Request
			bs = BeautifulSoup(send,'html.parser').find('font',face="Courier") # Parser Page
			return str(bs).replace('''<script class="stripe-button" data-amount="19900" data-currency="usd" data-description="Full report for 'google.com'" data-image="/images/ok.GIF" data-key="pk_live_ey9TT0KvaFQoLWRyDYg9oqQd" data-label="Download The Full Report for $199" data-name="ViewDNS.info" src="https://checkout.stripe.com/checkout.js"></script>''','')
		try:
			print '[*] Starting ... [*]'
			print '[*] ip/host %s is Activ [*]'%dom
			print '[*] Please White ... [*]'
			rand = str(randint(0,2000))
			fname = rand+'reverse_whois_'+dom.replace('.','_')+'.html' # File Name
			recv = psend(dom)
