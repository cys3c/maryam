...
def start():
	...
	host = raw_input(C+'[*]'+W+'asm>[find_shared_dns] Host/ip: ') # Target
	file = raw_input(C+'[*]'+W+'asm>[find_shared_dns] show File output?y/n ') # y = Save To File n = Show Recv
	dom = flib(host)
	...
	if chost(dom) == True: # Check For Host
		...
		try:
			...
			if (file == 'y' or file == 'Y'):
				...
				fname = rand+'find_shared_dns_'+dom.replace('.','_')+'.html' # File Name
